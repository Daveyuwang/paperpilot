...
export function SourcesView() {
  ...
  const [discoverOpen, setDiscoverOpen] = useState(sources.length === 0);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {/* Workspace Sources */}
        <section>
          <h3 className="sr-only">Workspace sources</h3>
          ...
          {filteredSources.length === 0 && sources.length === 0 ? (
            <div className="flex flex-col items-center py-8 text-center">
              ...
              <p className="text-[11px] text-surface-400 max-w-[200px]">
                Upload a PDF or search below to add sources.
              </p>
            </div>
          ...
          {discoverOpen && (
            <div className="space-y-3">
              ...
              {!searching && searchError && (
                <div className="flex items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700" role="alert">
                  <span>{searchError}</span>
                  <button className="font-semibold hover:text-red-900" onClick={handleSearch}>Retry</button>
                </div>
              )}

              {!searching && !searchError && searched && results.length === 0 && (
                <p className="text-xs text-surface-400 text-center py-4">No results. Try another query.</p>
              )}
...
function SourceRow({ source, workspaceId }: { source: WorkspaceSource; workspaceId: string }) {
  ...
  const { switchPaperAgenda } = useAgendaStore();
  const [labelOpen, setLabelOpen] = useState(false);
  const cfg = LABEL_CONFIG[source.label];

  const handleOpenInReader = useCallback(async () => {
    if (!source.paper_id) return;
    const result = await selectPaper(source.paper_id);
    if (result === null) return;
    setActiveViewerTab("reader");
    switchPaperAgenda(source.paper_id);
    setSelectedNav("reader");
  }, [source.paper_id, setActiveViewerTab, switchPaperAgenda, setSelectedNav, selectPaper]);
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {source.provider !== "upload" && (
            <span className="text-[10px] text-surface-400">{source.provider}</span>
          )}
          {source.provider === "upload" && (
            <span className="text-[10px] text-accent-500">PDF</span>
          )}
        </div>
      </div>
  ...
  return (
    <div className="px-3 py-2 rounded-lg border border-surface-100 bg-white hover:border-surface-200 transition-colors">
      <div className="flex items-start gap-2">
        <div
          className="min-w-0 flex-1 cursor-pointer rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-400"
          ...
          aria-controls={result.abstract ? abstractId : undefined}
        >
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-[10px] text-surface-400">{result.provider}</span>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {alreadySaved ? (
            <span className="p-1" title="In sources">
            ...
            <button
              ...
              aria-label={`Add ${result.title} to sources`}
            >
      ...
      {expanded && result.abstract && (
        <p id={abstractId} className="text-[11px] text-surface-500 mt-2 leading-relaxed">{result.abstract}</p>
      )}
    </div>
  );
}
